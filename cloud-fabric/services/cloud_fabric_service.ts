...
import { Auction, Bid, CreateAuctionRequest, PlaceBidRequest } from '../src/types/auction';
import { config } from '../src/config';

export class CloudFabricService {
  private ethereumAdapter: EthereumAdapter;
  private bigchainAdapter: BigchainDBAdapter;
  private activeAuctions: Map<string, Auction>;

  constructor() {
    this.ethereumAdapter = new EthereumAdapter();
    this.bigchainAdapter = new BigchainDBAdapter();
  ...
  async createAuction(req: Request, res: Response): Promise<void> {
    try {
      const { nftId, creator, minBid, startTime, endTime } = req.body as CreateAuctionRequest;
      ...
      if (duration < config.auction.minDuration) {
        res.status(400).json({
          success: false,
          error: `Auction duration must be at least ${config.auction.minDuration} seconds`,
        });
        return;
      }

      if (duration > config.auction.maxDuration) {
        res.status(400).json({
          success: false,
          error: `Auction duration cannot exceed ${config.auction.maxDuration} seconds`,
        });
        return;
      }
  ...
  async getAuction(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!auction) {
        const fetchedAuction = await this.bigchainAdapter.getAuction(id);
        auction = fetchedAuction ?? undefined;
      }
  ...
  async placeBid(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const { bidder, amount } = req.body as PlaceBidRequest;
      ...
      const isValid = await this.ethereumAdapter.validateBid(
        bidder,
        bidAmount,
  ...
  async closeAuction(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (auction.highestBidder) {
        ...
        const nftTxHash = await this.ethereumAdapter.transferNFT(
          auction.nftId,
          auction.creator,
        ...
        const tokenTxHash = await this.ethereumAdapter.transferTokens(
          auction.highestBidder,
          auction.creator,
          auction.currentBid
        );
      ...
      const finalizeTxId = await this.bigchainAdapter.finalizeAuction(
        auction.id,
        auction.currentBid,
        auction.highestBidder,
        transactionHash
